...
def  linear_regress_ridge():
    ...
    lr = Ridge(alpha=1)
    lr.fit(x_train,y_train)
    print "采用 岭回归，得到的线性模型的回归系数为：",lr.coef_
    ...
    print "采用岭回归方法，测试样本的房子预测价格是： ",y_predict_new
    print "采用岭回归方法的模型均方误差为：",mean_squared_error(std_y.inverse_transform(y_test),y_predict_new)
